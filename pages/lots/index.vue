      ...
      <b-row>
        <b-col cols="12">
          <b-card header-tag="header" footer-tag="footer">
            <!-- ヘッダー -->
            <div slot="header">
              <i class="fa fa-align-justify"></i> <strong>ロット管理</strong>
              <b-button variant="primary" class="float-right" href="/lots/new"><i class="fa fa-plus"></i> 新規追加</b-button>
            </div>
            <!-- /ヘッダー -->
            <div>
            ...
            <!-- 一覧 -->
            <b-table responsive="sm" :items="showLots" :fields="fields" :current-page="currentPage" :per-page="perPage">
...
<script>
  ...
  export default {
      ...
      showLots() {
        return this.lots.map(lot => {
          return {
            'id': lot.id,
            '名称': lot.name,
            '拠点名': lot.location.name,
            'ブランド名': lot.brand.name,
            '更新日': lot.updated_at
          };
        })
      },
    ...
    data () {
      return {
        fields: ['id', '名称', '拠点名', 'ブランド名', '更新日', 'controls'],
        currentPage: 1,
        perPage: 5,
        totalRows: 0,
      }
    ...
    methods: {
      ...
      onClickDestroy(lot) {
        if (confirm('本当に削除してもよろしいでしょうか。')) {
          const data = { lot: lot };
          // 削除処理
          this.deleteLot(cloneDeep(data));
        }
      },
      ...mapActions('lots', ['deleteLot'])
    }
  }
</script>
