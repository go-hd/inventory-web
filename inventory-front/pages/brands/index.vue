      ...
      <b-row>
        <b-col cols="12">
          <b-card header-tag="header" footer-tag="footer">
            <!-- ヘッダー -->
            <div slot="header">
              <i class="fa fa-align-justify"></i> <strong>ブランド管理</strong>
              <b-button variant="primary" class="float-right" href="/brands/new"><i class="fa fa-plus"></i> 新規登録</b-button>
            </div>
            <!-- /ヘッダー -->
            <div>
            ...
            <!-- 一覧 -->
            <b-table responsive="sm" :items="showBrands" :fields="fields" :current-page="currentPage" :per-page="perPage">
...
<script>
  ...
  export default {
      ...
      showBrands() {
        return this.brands.map(brand => {
          return {
            ...
            'コード': brand.code,
            '備考': brand.note,
            '更新日': brand.updated_at
          };
        })
      },
      ...mapGetters('brands', ['brands', 'alertMessage', 'alertStatus']),
    },
    data () {
      return {
        fields: ['id', '名称', 'コード', '備考', '更新日', 'controls'],
        currentPage: 1,
        perPage: 5,
        totalRows: 0,
      }
    ...
    methods: {
       ...
       * @param brand
       */
      onClickDestroy(brand) {
        if (confirm('本当に削除してもよろしいでしょうか。')) {
          const data = { brand: brand };
          // 削除処理
          this.deleteBrand(cloneDeep(data));
        }
      },
