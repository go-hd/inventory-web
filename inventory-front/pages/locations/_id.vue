      ...
      <b-row>
        <b-col cols="12">
          <b-card header-tag="header" footer-tag="footer">
            <!-- ヘッダー -->
            <div slot="header">
              <i class="fa fa-align-justify"></i> <strong>拠点管理 | 編集</strong>
            </div>
            <!-- /ヘッダー -->
            <!-- パン屑 -->
            ...
            <b-form-group>
              ...
              <b-form-input type="text" id="name" placeholder="拠点名" v-model="location.name"
            ...
            <b-form-group>
              ...
              <b-form-select id="company_id" :options="getCompanyOptions" v-model="location.company_id"
            ...
            <b-form-group>
              ...
              <b-form-select id="company_id" :options="getLocationTypeOptions" v-model="location.location_type_id" v-bind:class="{ 'is-invalid': errors.location_type_id }"></b-form-select>
...
<script>
  ...
  export default {
    ...
    async asyncData({ store, route }) {
      if (store.getters['locations/locations'].find(data => data.id === route.params.id)) {
        return
      }
    ...
    computed: {
       ...
       */
      location() {
        return cloneDeep(this.locations.find(data => data.id == this.$route.params.id));
      },
      /**
       * セレクトボックス用の会社一覧を取得
       *
      ...
      ...mapGetters('locations', ['locations', 'alertMessage', 'alertStatus', 'errors']),
    ...
    methods: {
      ...
      async onClickUpdate() {
        ...
        // 更新処理
        const response = await this.updateLocation(cloneDeep(data));
        // OKであれば拠点一覧へ遷移する
        if (response.status) {
